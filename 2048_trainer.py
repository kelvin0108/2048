...
# 其他参数
num_actions = 4  # R L U D
state_size = 16  # state=4x4
...
class DQN(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)  # 展平，變為 [batch_size, 16]
...
def select_action(state):
    ...
    if random.random() > eps_threshold:
        with torch.no_grad():
            state_flattened = state.view(-1).unsqueeze(0)  # 确保状态形状是 (1, 16)
            return policy_net(state_flattened).argmax().item()  # 选择 Q 值最大的动作
    else:
...
def optimize_model():
    ...
    batch_state = torch.stack(
        [s.unsqueeze(0) if len(s.shape) == 2 else s for s in batch_state],
        dim=0
    )
    ...
    # 展平状态为16维向量 [64, 16]
    batch_state = batch_state.view(batch_state.size(0), -1) 

    # 计算当前Q值
    state_action_values = policy_net(batch_state).gather(1, batch_action)

    # 计算下一个状态的最大Q值
    next_state_values = target_net(batch_next_state).max(1)[0].detach().unsqueeze(1)
...
for i_episode in range(num_episodes):
    ...
    for t in range(1000):  # 每局最多猜1000次
        # 选择动作
        action = select_action(state)

        # 执行动作，将 action 作为单个参数传递
        next_state, reward, done, info, biggest_num = env.step(action)
